...
describe("diamond_hands", () => {
  ...
  it("Should withdraw after timelock", async () => {
    ...
    const senderLamportsBefore = await program.provider.connection.getBalance(
      sender.publicKey
    );
    const receiverLamportsBefore = await program.provider.connection.getBalance(
      receiver.publicKey
    );
    ...
    const senderLamportsAfter = await program.provider.connection.getBalance(
      sender.publicKey
    );
    const receiverLamportsAfter = await program.provider.connection.getBalance(
      receiver.publicKey
    );
    ...
    assert.equal(
      senderLamportsAfter,
      senderLamportsBefore - bank.amount.toNumber()
    );
    assert.equal(
      receiverLamportsAfter,
      receiverLamportsBefore + bank.amount.toNumber()
    );
